...
Looking at \ref{tab} that we computed with magma we can see that in all the cases that the graph is $q+1$ regular except for
...
a reference \cite{•} where this fact is proved. We will not explain the proof for reason of space.
...
eigenvalues satisfy this bound. Anyway this bound holds for all eigenvalues, but this would require other 30 pages in the report.
